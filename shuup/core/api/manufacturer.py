...
class ManufacturerSerializer(serializers.ModelSerializer):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if request.method in ["POST"]:
            ...
            self.fields["path"] = serializers.CharField(required=False, write_only=True)
        else:
    ...
    def validate(self, data):
        if (data.get("path")):
            data["logo"] = filer_image_from_upload(self.context["request"], path=data["path"], upload_data=data["logo"])
        return data
