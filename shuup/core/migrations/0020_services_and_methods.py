...
def create_default_service_providers(apps, schema_editor):
    ...
    apps.get_model("shuup", "ShippingMethod").objects.filter(
    ...
    apps.get_model("shuup", "PaymentMethod").objects.filter(
...
def set_shop_for_methods(apps, schema_editor):
    shop = None
    method_models = [
        apps.get_model("shuup", "PaymentMethod"),
        apps.get_model("shuup", "ShippingMethod"),
    ]
