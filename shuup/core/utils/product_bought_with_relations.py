...
def add_bought_with_relations_for_product(product_id, max_quantity=10):
    ...
    related_product_ids_and_quantities = (
        OrderLine.objects.exclude(product_id=product_id)
        .filter(type=OrderLineType.PRODUCT, order_id__in=set(order_ids_to_check))
        .values("product_id")
        .annotate(total_quantity=Sum("quantity"))
        .order_by("-total_quantity")[:max_quantity]
    )
