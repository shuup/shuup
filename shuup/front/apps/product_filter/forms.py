...
class AttributesFilterSettingsForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for base_attr in BASIC_ATTRIBUTE_FIELDS:
            ...
            if ' id' in base_attr_label:
                base_attr_label = base_attr_label.replace(' id', '')
            self.fields[enabled_attr].label = _(base_attr_label)
...
class ProductFilterForm(ProductListFormModifier):
    ...
    def get_attributes(self, request):
        ...
        settings_attributes = ProductAttribute.objects\
            ...
            .filter(
                attribute_id__in=attr_filters_ids,
                untranslated_string_value__isnull=False
            )
    ...
    def get_price(self, request):
        price_ranges = []
        max_price = ShopProduct.objects.filter(
            shop=get_shop(request)
        ).aggregate(
            Max('default_price_value')
        )

        if max_price['default_price_value__max']:
    ...
    def get_fields(self, request, category=None):
        ...
        prices_ranges = self.get_price(request)
    ...
    def get_basic_attributes_query(self, data_key, data_value):
        ...
        if data_key in basic_attributes:
            basic_attributes[data_key].append(data_value)
        else:
            basic_attributes[data_key] = [data_value]
