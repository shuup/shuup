...
from shuup.utils.filer import (
    ensure_media_file, ensure_media_folder, ensure_media_folder_users,
...
def _get_folder_query_filter(shop, user=None):
    ...
    if user:
        query &= Q(media_folder__user_access=user)
    return query


def _get_folder_query(shop, user, folder=None):
...
def get_or_create_folder(shop, path, user=None):
    ...
    for folder in folders:
        if folder != "":
            ...
            media_folder = ensure_media_folder(shop, child)
            ensure_media_folder_users(media_folder, child.logical_path, user)
            parent = child
    return child
...
class MediaBrowserView(TemplateView):
    ...
    def handle_get_folders(self, data):
        ...
        users_root_folder = Folder.objects.filter(media_folder__root_folder_for=self.request.user).first()
        if users_root_folder:
            root_folders = get_cached_trees(
                Folder._tree_manager.filter(_get_folder_query_filter(shop, self.user) & ~Q(id=users_root_folder.id))
            )
            return JsonResponse({"rootFolder": filer_folder_to_json_dict(users_root_folder, root_folders)})
        else:
    ...
    def _create_folder(self, name, parent, shop):
        ...
        media_folder = ensure_media_folder(shop, folder)
        ensure_media_folder_users(media_folder, folder.logical_path, self.request.user)
    ...
    def handle_get_get_edit_url(self, data):
        shop = get_shop(self.request)
    ...
    def handle_post_rename_folder(self, data):
        ...
        message = _("Can't rename a this folder.")
        return JsonResponse({"success": False, "message": message})
    ...
    def handle_post_delete_folder(self, data):
        ...
        message = _("Can't delete a this folder.")
        return JsonResponse({"success": False, "message": message})
    ...
    def handle_post_rename_file(self, data):
        ...
        message = _("Can not rename a this file.")
        return JsonResponse({"success": False, "message": message})
    ...
    def handle_post_delete_file(self, data):
        ...
        message = _("Can not delete a this file.")
        return JsonResponse({"success": False, "message": message})
