...
from shuup.admin.utils.permissions import (
    ...
    get_permission_string_for_model, user_has_permission_to_view_kind
)
...
class AdminRegexURLPattern(RegexURLPattern):
    ...
    def _get_unauth_reason(self, request, *args, **kwargs):
        """
        ...
        missing_permissions = get_missing_permissions(request.user, self.permissions, request, **kwargs)
    ...
    def wrap_with_permissions(self, view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            unauth_reason = self._get_unauth_reason(request, *args, **kwargs)
            if unauth_reason:
...
def get_model_url(object, kind="detail", user=None, required_permissions=None, request=None):
    ...
    for module in get_modules():
        ...
        if user is None:
            ...
            # check if shop permission exists
            if user_has_permission_to_view_kind(user, object, kind):
                return url
            ...
            # check if user has permission to this object
            # TODO: FIX Broken
