...
from shoop.testing.factories import create_product, get_shop, get_default_shop
...
def test_rule_min_max():
    ...
    # rule.save()
    assert rule.postal_codes_min == "99501"
    assert rule.postal_codes_max == "99590"

    TaxRule.objects.create(postal_codes_pattern="123,456,88,99999,1", tax=tax)
    TaxRule.objects.create(postal_codes_pattern="1-99999", tax=tax)
    ...
    postal_code = "99510"
    assert TaxRule.objects.in_postalcode_range(postal_code).count() == 4

    rule.postal_codes_pattern = "20320,!20100"
    rule.save()
    assert not rule.postal_codes_min
    assert not rule.postal_codes_max
    assert TaxRule.objects.in_postalcode_range(postal_code).count() == 3

    postal_code = None
    assert TaxRule.objects.in_postalcode_range(postal_code).count() == 5
...
def test_rule_admin(rf, admin_user):
    ...
    postal_code = "99510"
    assert TaxRule.objects.in_postalcode_range(postal_code).count() == 1

    postal_code = "99500"
    assert not TaxRule.objects.in_postalcode_range(postal_code).count()
