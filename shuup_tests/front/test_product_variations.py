...
def test_variation_detail_view(client):
    ...
    volume = ProductVariationVariable.objects.create(
        identifier='volume', name='volume', product=parent, ordering=0,
    )
    for index, value in enumerate(['1ml', '2ml', '3ml']):
        ProductVariationVariableValue.objects.create(
            identifier=value, value=value, variable=volume, ordering=index,
        )

    color = ProductVariationVariable.objects.create(
        identifier='color', name='color', product=parent, ordering=1,
    )
    ...
    assert child.link_to_parent(parent, variables={
        volume: values[0],
        color: values[1],
    })
    ...
    variation_url = '{}?variation={}'.format(reverse(
        'shuup:product',
        kwargs=dict(pk=parent.pk, slug=parent.slug)
    ), child.sku)
