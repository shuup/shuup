...
@pytest.mark.django_db
def test_create_amount_refund(rf, admin_user):
    shop = get_default_shop()
    supplier = get_default_supplier()
    product = create_product(sku="test-sku", shop=shop, supplier=supplier, default_price=3.33)
    order = create_order_with_product(product, supplier, quantity=1, taxless_base_unit_price=1, shop=shop)
    ...
    data = {
        "form-0-line_number": "amount",
        "form-0-text": "Test refund",
        "form-0-amount": 1,
        "form-0-supplier": supplier.id,
        "form-INITIAL_FORMS": 0,
        "form-MAX_NUM_FORMS": 1000,
        "form-TOTAL_FORMS": 1,
        "form-MIN_NUM_FORMS": 0,
...
@pytest.mark.django_db
def test_create_to_big_amount_refund(rf, admin_user):
    shop = get_default_shop()
    supplier = get_default_supplier()
    product = create_product(sku="test-sku", shop=shop, supplier=supplier, default_price=3.33)
    order = create_order_with_product(product, supplier, quantity=1, taxless_base_unit_price=1, shop=shop)
...
@pytest.mark.django_db
def test_supplier_select_for_admin(rf, admin_user):
    shop = get_default_shop()
    supplier = get_default_supplier()
    product = create_product(sku="test-sku", shop=shop, supplier=supplier, default_price=3.33)
    order = create_order_with_product(product, supplier, quantity=1, taxless_base_unit_price=1, shop=shop)
    ...
    supplier_option_str = '$refund.find("select[id$=\'supplier\']"), true);'
    assert supplier_option_str in get_refund_view_content()
